...
/**
  * @brief  Sets RTC current date in seconds since 01.01.2000
  * @param  hrtc   pointer to a RTC_HandleTypeDef structure that contains
  *                the configuration information for RTC.
  * @param  sTime: Pointer to Time structure
  * @param  Format: Specifies the format of the entered parameters.
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_RTC_SetDateToRegister(RTC_HandleTypeDef *hrtc, RTC_DateTypeDef *sDate, uint32_t Format)
{
	...
	uint32_t counter_time = 0U;
	  ...
	  while (day != sDate->Date || month != sDate->Month || year != sDate->Year)
	  {
	    ...
	    /* Write date and time counter in RTC registers */
	    if (RTC_WriteTimeCounter(hrtc, counter_time) != HAL_OK)
	    {
	      return HAL_ERROR;
	    }
